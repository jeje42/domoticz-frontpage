...
Domotica.translate = {
	scanPage: function() {
		var all = document.getElementsByClassName("translated");

		for (var i=0, max=all.length; i < max; i++) {
		     Domotica.translate.translateElement(all[i])
		}
	},

	getLangageMap: function(){
	...
	translateElement: function(element){
		var langageMap = Domotica.translate.getLangageMap()
		var toTranslate = element.getAttribute("toTranslate")
		var tmp = undefined

		if(toTranslate){
			tmp = langageMap[toTranslate]
			if(tmp){
				element.innerHTML = element.innerHTML.replace(toTranslate, tmp)
			}
		}else{
			tmp = langageMap[element.innerHTML]
			if(tmp){
